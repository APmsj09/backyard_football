    ...
    const setupSide = (team, side, formationData, isOffense) => {
        ...
        sortedSlots.forEach(slot => {
            ...
            const pState = {
                ...
                weight: weight, height: height, playbookIQ: playerIQ,
                consistency: consistency, toughness: toughness, catchingHands: catching,
                tackling: tackling, blocking: blocking, blockShedding: blockShedding,
                throwingAccuracy: accuracy, coverage: coverage,
...
function executeAssignment(pState, assignment, offenseStates, LOS, playState, ballCarrierState) {
    ...
    // 4. QB SPY (Mirroring)
        ...
        // 💡 FIX: Spy the person with the ball behind the line, 
        // regardless of whether their role is 'QB' or 'RB'
        const target = ballCarrierState && ballCarrierState.y < LOS + 1
            ? ballCarrierState
            : offenseStates.find(p => p.slot === 'QB1'); // Fallback
...
function handleBallArrival(playState, carrier, playResult, gameLog) {
    ...
    const BASE_CATCH_RADIUS = 0.45; // 💡 FIX: Tighter radius stops teleports
    ...
    if (!ball.isThrowAway && playersInRange.length > 0) {
        ...
        // Resolve stats for the winner of the positional battle
        ...
        if (bestCandidate.isOffense) {
            ...
            if (defendersNear >= 2) catchScore *= 0.25;  // Double/Triple coverage!
        } else if (playersInRange.length > 1) {
            catchScore *= 0.85; // Defense traffic
        }

        if (playState.type === 'punt') catchScore += 15; // Punts are easier to catch
        ...
        if (Math.random() * 100 < catchScore) {
            ...
            // Snap the ball perfectly to the receiver's chest
            ...
            // 💡 FIX: MOMENTUM KILL (Stop WRs from running backwards after Curls/Comebacks)
            if (bestCandidate.vy < 0) {
                bestCandidate.vy *= 0.1;  // Erase backward momentum
                bestCandidate.vx *= 0.3;
                bestCandidate.vy += (1.5 + (agility / 100)); // Instant burst upfield
                bestCandidate.contactReduction = 0.5; // Help them plant their feet
            }

            // Force all defenders to "Locate" the new runner
...
function resolvePlay(offense, defense, offensivePlayKey, defensivePlayKey, context, options, isLive = false) {
    ...
    try {
        ...
        while (playState.playIsLive && playState.tick < playState.maxTicks) {
            ...
            // --- 1. RPO PRE-HANDOFF READ ---
            if (playState.handoffRequired && !playState.handoffOccurred && qb1.assignment === 'qb_rpo_read') {
                const conflictDef = defenseStates.find(d => (d.role === 'LB' || d.slot === 'DB3') && !d.isBlocked);
                if (conflictDef && (conflictDef.vy < -0.4 || conflictDef.y < playState.lineOfScrimmage + 1.5)) {
                    playState.handoffRequired = false; // QB PULLS THE BALL
                    qb1.action = 'qb_setup';
                    if (gameLog) pushGameLog(gameLog, `[RPO] 🧠 ${qb1.name} pulls the ball as the defender commits to the run!`, playState);
                }
            }
            ...
            // --- 3. POST-HANDOFF TRICK LOGIC (FLEA FLICKER) ---
            // This sits OUTSIDE the !handoffOccurred block
            const rbWithBall = playState.activePlayers.find(p => p.hasBall && p.fleaFlickerPhase === 'running_draw');
            if (rbWithBall) {
                if (playState.tick >= rbWithBall.fleaFlickerTimer) {
                    const qb = playState.activePlayers.find(p => p.slot === 'QB1');
                    rbWithBall.hasBall = false;
                    rbWithBall.isBallCarrier = false;
                    rbWithBall.fleaFlickerPhase = 'complete';

                    qb.hasBall = true;
                    qb.action = 'qb_setup';
                    qb.ticksInPocket = 40; // Simulate QB being "set" immediately
                    if (gameLog) pushGameLog(gameLog, `[Trick] 🏈 ${rbWithBall.name} pitches it back to ${qb.name}!`, playState);
                } else {
                    // Force the RB to run toward the line (draw the defense in)
                    rbWithBall.targetX = qb1.initialX;
                    rbWithBall.targetY = playState.lineOfScrimmage + 1;
                }
            }
            // --- C. AI TARGETING ---
