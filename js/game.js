 ...
 * @param {string} playType - 'pass' or 'run'.
 * @param {object} offensiveAssignments - Original offensive play assignments {slot: routeName}.
 * @param {string} defensivePlayCallKey - Key for the called defensive play (e.g., 'Cover_2_Zone'). // Added parameter
 */
function updatePlayerTargets(playState, offenseStates, defenseStates, ballCarrierState, playType, offensiveAssignments, defensivePlayCallKey) { // Added defensivePlayCallKey
    const timeDelta = TICK_DURATION_SECONDS;
    const qbState = offenseStates.find(p => p.slot?.startsWith('QB')); // Find QB state
    ...
    playState.activePlayers.forEach(pState => {
        ...
        if (pState.isOffense) {
            switch (pState.action) {
                ...
                case 'route_complete':
                    // --- Improvisation after route ---
                    // Find nearest defender to react to
                    const nearestDefender = defenseStates
                        .filter(d => !d.isBlocked && !d.isEngaged)
                        .sort((a, b) => getDistance(pState, a) - getDistance(pState, b))[0];

                    if (qbState?.action === 'qb_scramble') {
                        // QB is scrambling, move towards sideline QB is rolling towards, staying downfield
                        pState.targetX = qbState.targetX > CENTER_X ? FIELD_WIDTH - 5 : 5; // Move to sideline area
                        pState.targetY = Math.max(playState.lineOfScrimmage + 5, pState.y); // Stay past LoS, don't drift too far back
                    } else if (nearestDefender && getDistance(pState, nearestDefender) < 4.0) {
                        // Close defender, move away horizontally to create space
                        const moveXDir = pState.x > nearestDefender.x ? 1 : -1; // Direction away from defender X
                        pState.targetX = pState.x + moveXDir * 2; // Move 2 yards horizontally
                        pState.targetY = pState.y; // Stay roughly at same depth
                    } else {
                        // Relatively open, stay put to provide stationary target
                        pState.targetX = pState.x;
                ...
                case 'pass_block': case 'run_block':
                    // If engaged, maintain position relative to defender
                    if (pState.engagedWith) {
                         const engagedDefender = defenseStates.find(d => d.id === pState.engagedWith);
                         if(engagedDefender){
                             ...
                             pState.targetY = engagedDefender.y;
                              ...
                              pState.engagedWith = null;
                              pState.isEngaged = false; // Add this if using general engagement flag
                              // Fall through to find new target logic below
                         }
                    }
                    // If not engaged, find a target
                    if (!pState.engagedWith) {
                        let potentialTargets = [];
                        if (pState.action === 'pass_block') {
                            // Prioritize inside-out, closest rushers to QB
                            potentialTargets = defenseStates
                                .filter(d => !d.isBlocked && !d.isEngaged && (d.assignment === 'pass_rush' || d.assignment?.includes('blitz'))) // Target active rushers
                                .sort((a, b) => {
                            ...
                            // Prioritize defenders at LoS first, then LBs in run direction
                            const primaryTargets = defenseStates
                                 .filter(d => !d.isBlocked && !d.isEngaged && d.y < playState.lineOfScrimmage + 3 && Math.abs(d.x - pState.initialX) < 4) // Near LoS and original gap
                                 .sort((a, b) => getDistance(pState, a) - getDistance(pState, b));
                            const secondaryTargets = defenseStates
                                 .filter(d => !d.isBlocked && !d.isEngaged && d.y >= playState.lineOfScrimmage + 3 && d.y < playState.lineOfScrimmage + 8 && Math.abs(d.x - pState.initialX) < 6) // LBs further back
                                 .sort((a, b) => getDistance(pState, a) - getDistance(pState, b));
                             potentialTargets = primaryTargets.concat(secondaryTargets); // Combine lists, DL first
                        }
                        ...
                        if (targetDefender) {
                            ...
                            pState.targetX = pState.x + (targetDefender.x - pState.x) * interceptFactor;
                            pState.targetY = pState.y + (targetDefender.y - pState.y) * interceptFactor;
                        } else { // No immediate threat, hold position/zone
                            pState.targetX = pState.x;
                            pState.targetY = pState.y - (pState.action === 'pass_block' ? 0.05 : 0); // Tiny step back in pass pro
                        }
                    }
                    break;
                ...
                const nearestThreat = defenseStates
                    .filter(d => !d.isBlocked && !d.isEngaged &&
                                 d.y > pState.y && d.y < pState.y + threatDistance &&
                                 Math.abs(d.x - pState.x) < threatWidth)
                    .sort((a, b) => getDistance(pState, a) - getDistance(pState, b))[0];
                ...
                if (nearestThreat) {
                    ...
                    // Stronger avoidance effect, scales from 1.0 (at max dist) up to ~2.0 (very close)
                    const avoidStrength = 1.0 + (threatDistance - distanceToThreat) * 0.5;
                    targetXOffset = (pState.x >= nearestThreat.x) ? avoidStrength : -avoidStrength; // Move away
                }
                // --- Find Open Lane (Simple Placeholder - needs blocker info) ---
                // Placeholder: Use avoidance offset for now
                let bestLaneX = pState.x + targetXOffset;
                // --- End Find Lane ---
        ...
        // --- Defensive Logic ---
        else { // Defender logic (using the improved version from previous step)
             if (pState.isBlocked || pState.isEngaged) {
                // Hold position or fight block
                pState.targetX = pState.x; pState.targetY = pState.y;
             } else {
                 // Determine target based on assignment and play read
                 let target = null;
                 const assignment = pState.assignment;
                 const readPlayType = playState.tick > 7 ? playType : null;

                // --- Assignment-Based Targeting ---
            switch (true) { // Use switch(true) for cleaner startsWith/includes checks
                case assignment?.startsWith('man_cover_'):
                    const targetSlot = assignment.split('man_cover_')[1];
                    const assignedReceiver = offenseStates.find(o => o.slot === targetSlot);
                    if (assignedReceiver) {
                        target = assignedReceiver; // Store target player state for potential pursuit calculation later if needed
                        const speedDiff = (pState.speed || 50) - (target.speed || 50);
                        const yOffset = speedDiff < -15 ? -0.8 : speedDiff < -5 ? -0.3 : 0.3; // Trail/press
                        let xOffset = 0; // Leverage
                        if ((target.x < HASH_LEFT_X && pState.x > target.x) || (target.x > HASH_RIGHT_X && pState.x < target.x)) {
                            xOffset = (target.x < CENTER_X) ? 0.75 : -0.75; // Shade inside
                        }

                        // --- React to Ball Thrown in Man ---
                        if (isBallInAir && playState.ballState.targetPlayerId === target.id) {
                            // Ball thrown at assigned receiver - Undercut/Attack ball
                            pState.targetX = ballPos.x; // Target ball location directly
                            pState.targetY = ballPos.y;
                            target = null; // Target set directly, skip pursuit calc below
                        } else if (isBallInAir && getDistance(pState, ballPos) < 15) {
                            // Ball thrown nearby - Peel off towards ball
                            pState.targetX = ballPos.x;
                            pState.targetY = ballPos.y;
                            target = null; // Target set directly
                        } else {
                            // No ball thrown or thrown far away - Maintain leverage on receiver's target destination
                            // Target receiver's target coordinates + offsets for pursuit calculation
                            target = {
                                x: target.targetX + xOffset,
                                y: target.targetY + yOffset,
                                speed: target.speed, // Pass needed info for pursuit calc
                                targetX: target.targetX, // Pass target's target
                                targetY: target.targetY,
                                fatigueModifier: target.fatigueModifier
                            };
                            // Pursuit logic below will handle setting pState.targetX/Y
                        }
                    } else { // Receiver not found?
                        target = {x: pState.x, y: pState.y + 0.5}; // Target point: Drift back slightly
                    }
                    break; // Exit switch

                case assignment?.startsWith('zone_'):
                    const zoneCenter = getZoneCenter(assignment, playState.lineOfScrimmage);
                    target = zoneCenter; // Default: Target zone center
                    ...
                    if (isBallInAir && isPlayerInZone({x: ballPos.x, y: ballPos.y}, assignment, playState.lineOfScrimmage) ) {
                        const targetReceiverState = playState.activePlayers.find(p => p.id === playState.ballState.targetPlayerId);
                        target = targetReceiverState || ballPos; // Prioritize receiver if found, else ball location
                        // If targeting the ball directly (receiver missed/not found), set target coords now
                        if (!targetReceiverState) {
                           pState.targetX = target.x; pState.targetY = target.y;
                           target = null; // Prevent pursuit calc on ball pos
                        } // else target receiver for pursuit calc
                    } else if (carrierInZone) {
                        target = ballCarrierState; // Underneath zones prioritize carrier
                    } else if (threatsInZone.length > 0) {
                        threatsInZone.sort((a,b)=> getDistance(pState, a) - getDistance(pState, b)); // Closest first
                        const primaryThreat = (assignment.includes('deep') && threatsInZone.length > 1) ? threatsInZone.sort((a,b) => b.y - a.y)[0] : threatsInZone[0]; // Deep zones prio deepest
                        target = primaryThreat;
                    } // Else: continue targeting zoneCenter (target already set)

                    // Target is now either playerState, {x,y} point, or null (if ball targeted directly)
                    // Pursuit logic below will handle if target is playerState
                    break;

                case assignment === 'pass_rush': case assignment === 'blitz_gap': case assignment === 'blitz_edge':
                    target = qbState; // Target QB state
                    if (target) {
                        // --- Simple Blocker Avoidance ---
                        const blockerInPath = offenseStates.find(o =>
                            (o.action === 'pass_block' || o.action === 'run_block') && !o.engagedWith &&
                            getDistance(pState, o) < 2.0 && Math.abs(o.x - pState.x) < 1.0 &&
                            ((pState.y < o.y && o.y < target.y) || (pState.y > o.y && o.y > target.y))
                        );
                        if (blockerInPath) {
                            const avoidOffset = (pState.x > blockerInPath.x) ? 1.0 : -1.0;
                            // Set target point slightly around blocker, still aiming for QB depth
                            target = { x: pState.x + avoidOffset * 2, y: target.y }; // Target point, not player
                        } // Else target remains qbState for pursuit logic
                    } else { target = {x: pState.x, y: pState.y}; } // Hold if no QB
                    break;

                case assignment?.startsWith('run_gap_'): case assignment?.startsWith('run_edge_'):
                    const runTargetPoint = zoneBoundaries[assignment];
                    const ballSnapX = playState.activePlayers.find(p=>p.slot === 'OL2')?.initialX || CENTER_X;
                    // Default target is the gap point
                    target = runTargetPoint ? { x: ballSnapX + runTargetPoint.xOffset, y: playState.lineOfScrimmage + runTargetPoint.yOffset } : {x: pState.x, y: pState.y};

                    // React aggressively to nearby ball carrier or clear run read
                    if ((readPlayType === 'run' && ballCarrierState) || (ballCarrierState && getDistance(pState, ballCarrierState) < 6)) {
                        target = ballCarrierState; // Pursue carrier state
                    } else if (readPlayType === 'pass') {
                        // If read pass, convert to pass rush
                         pState.assignment = 'pass_rush'; // Change assignment mid-play
                         target = qbState; // Target QB state
                    } // Else hold original gap target (target point already set)
                    break;

                 case assignment === 'spy_QB':
                     target = qbState;
                     if (target) {
                        // Stay ~5-8 yards away from QB, mirroring horizontal movement
                        const desiredY = target.y + 7; // Maintain depth relative to QB
                        const desiredX = target.x; // Mirror X
                        // Set target point directly
                        pState.targetX = desiredX; pState.targetY = desiredY;
                        target = null; // Prevent pursuit calc
                     } else { target = {x: pState.x, y: pState.y}; } // Hold if no QB
                     break;

                default: // Default / Read assignment
                     if (isBallInAir) { target = ballPos; } // Target ball landing spot {x, y}
                     else if (ballCarrierState) { target = ballCarrierState; } // Pursue carrier state
                     else if (qbState?.action === 'qb_scramble') { target = qbState;} // Contain QB scramble state
                     else { target = {x: pState.x, y: pState.y + 0.05}; } // Slow read step/drift {x, y}
                     break;
            } // --- End Assignment Switch ---


            // --- Set Target Coordinates ---
            // Apply pursuit only if target is a player state and wasn't handled directly above
            if (target && target.speed !== undefined) {
                 // --- Enhanced Pursuit Angle ---
                 const targetSpeed = target.speed || 50;
                 const ownSpeedYPS = Math.max(1, (pState.speed / 10 * pState.fatigueModifier));
                 const distToTarget = getDistance(pState, target);
                 ...
                 // --- End Pursuit Angle ---
            } else if (target) { // Target is a fixed point {x, y} and wasn't handled directly above
                 pState.targetX = target.x;
                 pState.targetY = target.y;
            } else if (!target && !assignment?.startsWith('man_cover_') && !(assignment?.startsWith('zone_') && isBallInAir) && assignment !== 'spy_QB') {
                 // If no target was determined and not handled directly, hold position
                 pState.targetX = pState.x;
                 pState.targetY = pState.y;
            } // else target was set directly in Man/Zone(Ball)/Spy cases
