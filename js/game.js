...
// --- FIX: REMOVED DUPLICATE getPlayersForSlots FUNCTION ---
// The second, less-safe version that was here has been removed.


// --- FIX: ADDED MISSING getPlayerBySlot FUNCTION ---
/**
 * Gets a single, healthy player for a specific slot, finding a sub if needed.
 * @param {object} team - The team object.
 * @param {string} side - 'offense' or 'defense'.
 * @param {string} slot - The specific slot (e.g., 'QB1').
 * @param {Set} usedPlayerIdsThisPlay - A Set of player IDs already used in this play.
 * @returns {object|null} The player object or null if no one is available.
 */
function getPlayerBySlot(team, side, slot, usedPlayerIdsThisPlay) {
    if (!team || !team.depthChart || !team.depthChart[side] || !team.roster) {
        console.error(`getPlayerBySlot: Invalid team data for slot ${slot}.`);
        return null;
    }
    const position = slot.replace(/\d/g, ''); // 'QB1' -> 'QB'
    const starterId = team.depthChart[side][slot];
    let player = team.roster.find(p => p.id === starterId);
    ...
    // If a valid player (starter or sub) is found
    if (player && !usedPlayerIdsThisPlay.has(player.id)) {
        usedPlayerIdsThisPlay.add(player.id);
        return player;
    }
    return null; // No one available for this slot
}

// --- FIX: ADDED MISSING findEmergencyPlayer FUNCTION ---
/**
 * Finds *any* healthy player on the roster for a given position if the designated slots are empty.
 * Used as a last resort.
 * @param {string} position - The base position needed (e.g., 'QB').
 * @param {object} team - The team object.
 * @param {string} side - 'offense' or 'defense'.
 * @param {Set} usedPlayerIdsThisPlay - A Set of player IDs already used in this play.
 * @returns {object|null} The player object or null if no one is available.
...
function findEmergencyPlayer(position, team, side, usedPlayerIdsThisPlay) {
    ...
    // Find the best *available* healthy player for that position
...
                     primaryDefender.gameStats.interceptions = (primaryDefender.gameStats.interceptions || 0) + 1;
                        playState.turnover = true; // --- FIX: Added turnover flag ---
                 } else {
...
    [...dls, ...lbs, ...dbs].forEach(p => { if (p) p.fatigue = Math.min(100, (p.fatigue || 0) + 5); }); // Safe access to fatigue
    ...
    // --- FIX: Use getPlayerBySlot for QB and findEmergencyPlayer as fallback ---
...
       const qbPower = (qb.attributes?.physical?.strength || 0) + (qb.attributes?.physical?.weight || 0) / 5 * getFatigueModifier(qb);
        // --- FIX: Use getPlayerBySlot for DL stopper ---
...
                 const routeName = assignments[slot]; if (!routeName || routeName.toLowerCase().includes('block')) return; const routeInfo = routeTree[routeName]; if (!routeInfo) return;
                // --- FIX: Use getPlayerBySlot to find receiver ---
                 const receiver = getPlayerBySlot(offense, 'offense', slot, new Set()); // Use temporary Set as receiver isn't 'used' yet for coverage
                if (!receiver) return;
...
        if (type === 'run') { /* ... (Setup logic with safe access) ... */
            // --- FIX: Use getPlayerBySlot for RB and findEmergencyPlayer as fallback ---
             playState.ballCarrier = getPlayerBySlot(offense, 'offense', 'RB1', usedPlayerIds_O) || findEmergencyPlayer('RB', offense, 'offense', usedPlayerIds_O)?.player;
...
     if (!team || !team.roster || !game || !game.freeAgents || !team.coach) return; // Safety checks
    const MIN_HEALTHY_PLAYERS = 7; // Define minimum

    // --- FIX: Use a loop to sign players until minimum is met or FAs run out ---
    let healthyCount = team.roster.filter(p => p && p.status?.duration === 0).length;

    // Only sign if below minimum players and FAs are available
...
         if (!bestFA) break; // Should not happen if freeAgents.length > 0, but safety check
...
            team.roster.push(bestFA);
            healthyCount++; // Increment count as a player was added
            console.log(`${team.name} signed temporary player ${bestFA.name}`); // Log AI action
        }
        // If the attempt failed, the loop continues checking if more FAs are available
    }
    // --- End FIX ---
}
